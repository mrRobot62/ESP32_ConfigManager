...
int ConfigManager::getNamespaceKeys(const String& ns, String *keys, size_t maxKeys) {
    preferences.begin(ns.c_str(), true);
    size_t index = 0;
    nvs_iterator_t it = nvs_entry_find(NVS_DEFAULT_PART_NAME, ns.c_str(), NVS_TYPE_ANY);
    while (it != NULL && index < maxKeys) {
        nvs_entry_info_t info;
        nvs_entry_info(it, &info);
        keys[index++] = String(info.key);
        it = nvs_entry_next(it);
    }
    preferences.end();
